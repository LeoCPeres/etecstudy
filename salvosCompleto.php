...
<div class="row">
    ...
    $top4Salvos = $salvos->consultarSalvos();
    if ($top4Salvos != null) {
        foreach ($top4Salvos as $consultarSalvos) { ?>
    <div class="col-sm-3">
        <div class="card shadow" style="border: none !important;">
            <?= $consultarSalvos['imagem'] != null ? '<img class="card-img-top img-fluid"
src="./img/capas/' . $consultarSalvos['imagem'] . '"
alt="Card image cap" style="height: 200px; object-fit: cover;">' : '<div class="d-flex align-items-center justify-content-center">Image not found</div>' ?>
            <div class="card-body">
                <h5 class="card-title etc"><?php $tituloCortado = substr($consultarSalvos['titulo'], 0, 18);
                                                    echo ($tituloCortado . '...')  ?></h5>
                <p class="card-text etc" style="height: 110px"><?php $descricaoCortada = substr($consultarSalvos['descricao'], 0, 85);
